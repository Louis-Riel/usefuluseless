...
#include "/home/nix/Documents/PlatformIO/Projects/esp-iot-solution/components/motor/stepper/a4988/include/iot_a4988.h"
#include "unity.h"
...
#endif
static const char* TAG = "a4899";

a4899::a4899(int stepPin, int dirPin){
    this->dirPin=dirPin;
    this->stepPin=stepPin;


//    CA4988Stepper stepper(this->stepPin,this->dirPin,800);
//    int steps = 800*3;
//    int rpm = 60*5;
//
...
//    vTaskDelay(1000/portTICK_RATE_MS);
    ...
    ESP_ERROR_CHECK(gpio_set_direction((gpio_num_t)this->dirPin, GPIO_MODE_OUTPUT));
    ESP_ERROR_CHECK(gpio_set_direction((gpio_num_t)this->stepPin, GPIO_MODE_OUTPUT));

    ESP_LOGD(TAG,"%s dir on",__func__);
    ESP_ERROR_CHECK(gpio_set_level((gpio_num_t) this->dirPin,1));
    ...
    for (int delay=0; delay < 4; delay++){
        ESP_ERROR_CHECK(gpio_set_level((gpio_num_t) this->dirPin,direction));
        direction=direction?0:1;
        ESP_LOGD(TAG,"%s %d 10 start spin",__func__,delay);
        for (int idx=0; idx < 1000; idx++) {
            ESP_ERROR_CHECK(gpio_set_level((gpio_num_t) this->stepPin,0));
            ESP_ERROR_CHECK(gpio_set_level((gpio_num_t) this->stepPin,1));
            if (delay)
                vTaskDelay(delay);
        }
        ESP_LOGD(TAG,"%s done spin",__func__);
    }
